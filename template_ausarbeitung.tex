...
\begin{algorithm}
	...
	\label{alg:BFS}
	\begin{algorithmic} []
		\REQUIRE Kapazitätenarray $cap$, Kostenarray $cost$, Quellknoten $src$, Senkenknoten $sink$
		%\ENSURE BFS
        ...
        \STATE $x$ = $sink$
        \IF{flow[x][dad[x]] \neq 0}
        \STATE $val$ = flow[x][dad[x]]
        \ELSE
        \STATE $val$ = cap[dad[x]][x] - flow[dad[x]][x]
        \ENDIF
    
        \WHILE{$x$ != $src$}
        \STATE $amt = \min( amt, val  )$
