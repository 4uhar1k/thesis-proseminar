...
Ian Kiprono, Senior Software Developer bei PInterest, hat einen Artikel zu diesem Thema auf der Webseite geeksforgeeks.com hochgeladen. Lasst uns, mithilfe von obengenanntem Artikel\cite{GeeksforGeeks2023}, zuerst das Algorithmus schrittweise aufteilen. Die grundsätzliche Idee ist wie folgt: (übersetzt durch DeepL):
\begin{enumerate}
    \item Gegeben seien der Quellknoten $\mathbf{S}$ und der Senkenknoten $\mathbf{T}$. Wir haben eine Hilfsfunktion, die überprüft, ob es noch einen gültigen Pfad von $\mathbf{T}$ nach $\mathbf{S}$ gibt. Wenn es keinen mehr gibt, terminiert der Algorithmus.
    \item Soweit es einen gültigen Pfad gibt, wählen wir einen Pfad von $\mathbf{T}$ nach $\mathbf{S}$ (entweder random oder mit den minimalen Kosten). Wir berechnen den maximalen Fluss, den wir entlang dieses Pfades senden können.
    \item Nachdem wir einen Pfad ausgewählt und den Fluss berechnet haben, sollen wir diesen Pfad aktualisieren, indem wir die Kapazität aller Kanten aktualisieren. 
...
\begin{enumerate}
    ...
    \item search() -
\end{enumerate}
Insgesamt bekommen wir die Laufzeit von $\mathbf{O(V^{2} \cdot E^{2})}$ und den Speicherverbrauch von $\mathbf{O(V)}$. Dieser Algorithmus passt also ganz gut für Lösung des Problems in relativ kleinen Graphen. Es gibt aber eine andere Lösung, die viel effizienter ist.
