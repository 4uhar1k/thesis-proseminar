...
\begin{enumerate}
    \item Gegeben sei ein Graph. Jede Kante in diesem Graph hat ihre Kosten \item Gegeben seien der Quellknoten $\mathbf{src}$ und der Senkenknoten $\mathbf{sink}$. Wir haben eine Hilfsfunktion, die überprüft, ob es noch einen gültigen Pfad von $\mathbf{sink}$ nach $\mathbf{src}$ gibt. Wenn es keinen mehr gibt, terminiert der Algorithmus.
