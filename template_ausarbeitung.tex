...
Jetzt betrachten wir, wie Min-Cost Flow Algorithmus am besten implementiert werden kann. Lasst uns, mithilfe von Artikel auf der Website geeksforgeeks.com \cite{GeeksforGeeks2023}, zuerst das Algorithmus schrittweise definieren:
\begin{enumerate}
    \item Speichern Sie die Kapazität einer Kante und die Kosten dieser Kante in zwei separaten Arrays.
    \item Gegeben seien der Quellknoten S und der Senkenknoten T, die ausgewählte Kante $\mathbf{p_i}$, die Nachfragknoten $\mathbf{d_a}$ und die Entfernung zwischen den Knoten $\mathbf{dist}$. Suchen Sie, ob es möglich ist, einen Fluss von S nach T zu haben.
    \item Wenn ein Fluss existiert, berechnen Sie die Entfernung, Wert = dist + $\mathbf{p_i}$ – $\mathbf{p_i}$[k] - cost[k].
    \item Vergleichen Sie die Entfernungswerte in dist[ ] mit dem Wert und aktualisieren Sie so lange, bis der minimale Fluss erreicht ist.
\end{enumerate}
