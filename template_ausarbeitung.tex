...
\begin{enumerate}
    ...
    \item Hilfsfunktion, die überprüft, ob es immer noch einen gültigen Pfad in dem Graph gibt. (search())
\end{enumerate}
\begin{algorithm}
	\caption{maxFlow}
	\label{alg:BFS}
...
Lasst uns zuerst mit dem Algorithmus anfangen, das wir früher implementiert haben. Beginnen wir mit dem Teil, wo wir den möglichstgroßen Fluss berechnen, also maxFlow(). Was machen wir da eigentlich?
\begin{enumerate}
    \item 
\end{enumerate}
...
\end{itemize}
Da das Problem schon seit langer Zeit geforscht wurde, gibt es mehrere Lösungen, die für polynomielle Zeit ausgeführt werden
\begin{center}
    \begin{tabular}{|c|c|c|}
\hline
Jahr & Laufzeit & Quelle \\ \hline
1972 & O(E \log U) * SP & Edmonds und Karp \\ \hline
1980 & O(E \log C) * MF & Röck \\ \hline
1984 & O(E^{2} \log V) * SP & Orlin, Fujishige \\ \hline
1985 & O(E^{2} \log V) * MF & Tardos \\ \hline
1986 & O(V^{2} \log V) * SP & Galil und Tardos \\ \hline
1988 & O(E \log V) * SP & Orlin \\ \hline
\end{tabular}
\end{center}

\subsection{Algorithm von Orlan (1988)}
