...
In meiner Ausarbeitung geht es um Min-Cost Max-Flow - ein Problem, das eine Grundlage bei der Lösung verschiedenster Aufgaben sein kann und dabei ganz leicht zu verstehen und implementieren ist. Ich werde heute Lösungsimplementierung von diesem Problem zeigen und erklären. Außerdem will ich paar Beispielaufgaben, u.a. aus dem echten Programmierwettbewerb vorzeigen, die mithilfe von Min-Cost Flow gelöst werden können.
\end{abstract}

\section{Einleitung}
\label{sec:intro}

  Graphen sind eine grundlegende Datenstruktur in der Programmierung. Sie ermöglichen es, komplexe reale Probleme zu modellieren und dadurch besser zu analysieren und zu verstehen.  Ein bekanntes Problem aus der Graphentheorie ist das Min-Cost-Max-Flow-Problem. In meiner Arbeit betrachte ich dieses Problem näher.
...
\subsection{Wichtiger Aspekt}
Jetzt betrachten wir, wie Min-Cost Flow Algorithmus am besten implementiert werden kann. Bevor wir mit der Implementierung beginnen, sollen wir einen Aspekt betrachten: die Kosten können auch negativ sein.\cite{Halim2018} \newline Warum ist das wichtig? In manchen Situationen können negative Zyklen entstehen. Sie sollten beseitigt werden, weil, wenn der gesamte Fluss diesen Zyklus durchlaufen muss, verringern sich die Gesamtkosten mit jedem abgeschlossenen Zyklus. Dies würde zu einer Endlosschleife führen, wenn man die Gesamtkosten minimieren möchte. \cite{GeeksforGeeks2023}
...
\end{algorithm}
%\newpage
\section{Algorithmus von Orlin (1988)}
Orlins Algorithmus gilt als der effizienteste nicht-randomisierte Algorithmus für Min-Cost Flow Problem. \cite{Cruz2023} In dieser Abteilung betrachten wir diese Lösung näher. \cite{orlin1988}\newline
Wir sollen zwei Schlüsselbegriffe definieren:
\begin{itemize}
    \item ex(v) = (eingehender Fluss) - (ausgehender Fluss) - d(v), wobei d(v) für Nachfrage des Knoten v steht.
    \item $\Delta = \text{größte Zweierpotenz} \leq U$, wobei $U$ für die maximale Kapazität im Graph steht.
\end{itemize}
Außerdem hat jeder Knoten v seinen Potential p(v), jede Kante hat auch reduzierte Kosten, die mit folgender Formel berechnet werden können: \newline
\begin{center}
    $\mathbf{c_p(v,w) = c(v,w) + p(v) - p(w)}$
\end{center}
Es gibt auch einen Parameter ${\varepsilon}$. Der spielt die wichtigste Rolle im Algorithmus. Am Anfang gilt ${\varepsilon = C}$, wobei C für die maximale Kantenkosten steht.  
Jetzt lasst uns den Algorithmus schrittweise aufteilen:
\begin{enumerate}
    \item Wir sollen mit einem aktiven Knoten starten. Ein Knoten ist aktiv, wenn $\mathbf{ex(v) \ge \Delta}$.
    \item Wir suchen jetzt einen Pfad nach Senke. Dabei dürfen nur die Kanten verwendet werden, für die $\mathbf{c_p(v,w) \leq 0}$ gilt.
    \item Auf unserem Pfad können drei verschidene Fälle entstehen:
    \begin{itemize}
        \item Wir haben einen Knoten mit $\mathbf{ex(v) < 0}$ erreicht. In diesem Fall sollen wir entlang des Pfades genau $\Delta$ Fluss schicken.
        \item Es gibt keine passende Kante mehr. In diesem Fall erneuern wir den Potenzial von dem Knoten v: $\mathbf{p(v) = max(p(w) - c(v,w) - \varepsilon)}$.
        \item Es gibt keinen aktiven Knoten mehr. Dann $\mathbf{\Delta = \Delta / 2}$.
    \end{itemize}
    \item Nach jeder Phase wird ${\varepsilon = \varepsilon / 2}$
\end{enumerate}
Der Algorithmus stoppt, wenn $\mathbf{\varepsilon < \frac{1}{2n}}$.
...
Lasst uns zuerst mit dem naiven Algorithmus anfangen, den wir früher in dieser Arbeit implementiert haben.
\begin{enumerate}
...
\subsection{Laufzeit von Orlin}
Jetzt zum Algorithmus von Orlin.
\begin{enumerate}
    \item Wir haben $\mathbf{O(V)}$ Augmentierungen pro Phase.
    \item Wir haben insgesamt $\mathbf{O(\log U)}$ Phasen. Dabei steht $\mathbf{U}$ für die maximale Kapazität im Graph.
\end{enumerate}
Insgesamt bekommen wir also $\mathbf{O(V\log U)}$. Orlins Algorithmus ist bestimmt schneller, als die naive Implementierung, was wir auch in den Experimenten sehen können.
\subsection{Experimente}

    \begin{center}
    \begin{tabular}{|c|c|c|c|}
\hline
Knoten & Kanten & Naiv & Orlin \\  \hline
5 & 20 & $<0.1$ ms & $<0.01$ ms\\ \hline
10 & 40 & $0.3$ ms & $<0.02$ ms\\ \hline
20 & 80 & 2 ms & 0.05 ms\\ \hline
100 & 400 & 0.3 s & 0.4 ms\\ \hline
500 & 2000 & 31 s & 16 ms\\ \hline
1000 & 4000 & 389 s & 46 ms\\ \hline
\end{tabular}
\end{center}
 Wir haben also eine Bestätigung bekommen, dass Orlins Algorithmus entscheidend schneller ist. In den kleineren Graphen gibt es keinen relevanten Unterschied zwischen den Algorithmen. Ich würde für die kleineren Graphen die naive Implementierung vorziehen, weil sie einfacher zu verstehen ist.\newline In den Programmierwettbewerben wird aber natürlich Algorithmus von Orlin oder einfachere Orlin-basierte Algorithmen verwendet.

\section{Beispiele}
Hier ist ein kurzer Überblick zu einer echten Programmieraufgabe. \cite{ChWall} \newline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=12cm]{figures/great wall.png}
    \caption{Chinesische Mauer Eingabe \cite{ChWall}}
\end{figure}
Diese Aufgabe ist ein typischer Beispiel eines Problems, das mithilfe von Min-Cost Max-Flow gelöst werden kann. Da, laut der Bedingung, $\mathbf{3 \leq n \leq 1000}$, sollen wir hier Orlin-Algorithmus verwenden. Am Ende bekommen wir folgende Lösung:
\begin{figure}[H]
    \centering
    \includegraphics[width=12cm]{figures/great wall losung.png}
    \caption{Chinesische Mauer Lösung \cite{ChWall}}
\end{figure}
...
In dieser Ausarbeitung ging es um Min-Cost Max-Flow Problem. Wir haben sowie naive, als auch effiziente Implementierung betrachtet, die Laufzeit von den beiden analysiert und die Beispielaufgaben gesehen. Ich hoffe, dass meine Ausarbeitung mehr Verständnis zu diesem Thema gebracht hat. 
...
\begin{itemize}
    ...
    \item ChatGPT - sprachliche Unterstützung, Hilfe mit LaTex
    \item Google Scholar - Literatursuche
\end{itemize}
