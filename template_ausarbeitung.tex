...
Ian Kiprono, Senior Software Developer bei PInterest, hat einen Artikel zu diesem Thema auf der Webseite geeksforgeeks.com hochgeladen. Lasst uns, mithilfe von obengenanntem Artikel\cite{GeeksforGeeks2023}, zuerst das Algorithmus schrittweise definieren (übersetzt durch DeepL):
\begin{enumerate}
...
\begin{itemize}
    \item Shortest $(s,t)$-path Problem – Wir haben einen Digraphen $G$, zwei bestimmte Knoten $\mathbf{s}$ und $\mathbf{t}$ und einen Kostenvektor $c \in \mathbf{Q}^m$. Die Aufgabe besteht darin, einen Pfad von $\mathbf{s}$ nach $\mathbf{t}$ mit minimalen Gesamtkosten zu finden.
    
    \item Transshipment problem –  die Variante von MCF, bei der keine Bogenkapazitäten vorhanden sind (oder, gleichbedeutend, alle Bogenkapazitäten sehr groß sind) 
    
    \item Negative-cycle detection problem –  we have a digraph $G$ and a cost vector $c \in \mathbf{Z}^m$. The task is to check whether
 there exists a directed cycle in G whose total cost is negative.
\end{itemize}
...
\begin{algorithm}
	...
	\label{alg:BFS}
	\begin{algorithmic} []
		\REQUIRE Kapazitätenarray $cap$, Kostenarray $cost$, Quellknoten $q$, Senkenknoten $s$
		\ENSURE BFS
